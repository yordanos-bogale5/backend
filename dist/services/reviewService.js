...
const approvalPersistence_1 = require("./approvalPersistence");
class ReviewService {
    constructor(approvalPersistence) {
        this.reviews = new Map();
        this.approvalPersistence = approvalPersistence || new approvalPersistence_1.ApprovalPersistenceService();
    }
    /**
    ...
    filterReviews(filters) {
        ...
        if (filters.category) {
            filtered = filtered.filter(r => r.categories.some(cat => cat.category.toLowerCase() === filters.category.toLowerCase()));
        }
        if (filters.channel) {
        ...
        if (filters.propertyName) {
            filtered = filtered.filter(r => r.propertyName.toLowerCase().includes(filters.propertyName.toLowerCase()));
        }
        if (filters.status) {
     ...
     * Sort reviews
     */
    sortReviews(reviews, sortBy, sortOrder) {
     ...
     */
    extractTopIssue(propertyName) {
        const allReviews = this.getAllReviews();
        const problematicReviews = allReviews.filter(r => r.propertyName === propertyName &&
        ...
        const topIssue = issues.reduce((max, current) => current[1] > max[1] ? current : max);
     ...
     */
    calculateStatus(averageRating, approvalRatio) {
